 ...
 * @property integer $secondary_photo_id
 ...
 * @property Photo $secondaryPhoto
 * @property Review[] $reviews
 * @property Review[] $activeReviews
 */
class Product extends ActiveRecord
{
    ...
    public static function create($brandId, $categoryId, $code, $name_ru, $name_en, $description_ru, $description_en, $quantity, Meta $meta): self
    {
        $product = new static();
    ...
    public function edit($brandId, $code, $name_ru, $name_en, $description_ru, $description_en, Meta $meta): void
    {
    ...
    public function changeQuantity($quantity): void
    {
        if ($this->modifications) {
            throw new DomainException('Измените количество единиц в модификациях товара.');
        }
    ...
    {
        if ($modificationId) {
            if ($quantity > $this->getModification($modificationId)->quantity) {
                throw new DomainException('Доступно только ' . $this->getModification($modificationId)->quantity . ' единиц этого товара.');
            }
        }
        if ($quantity > $this->quantity) {
            throw new DomainException('Доступно только ' . $this->quantity . ' единиц этого товара.');
        }
        return true;
    }
...
//        if ($quantity > $this->quantity) {
//            throw new \DomainException('Доступно только ' . $this->quantity . ' едениц товара ' . $this->name);
//        }
...
//            $this->recordEvent(new ProductAppearedInStock($this));
    ...
    {
        return $this->meta->getTitle() ?: $this->name;
    }
    ...
    public function getModificationByName($name): Modification
    {
        foreach ($this->modifications as $modification) {
            if ($modification->isNameEqualTo($name)) {
                return $modification;
            }
        }
        throw new DomainException('Modification is not found.');
    ...
    public function getModificationByCode($code): Modification
    {
        foreach ($this->modifications as $modification) {
    ...
    public function changeModificationQty($id, $quantity)
    {
        $modifications = $this->modifications;
        foreach ($modifications as $i => $modification) {
            if ($modification->isIdEqualTo($id)) {
                $modification->changeQty($quantity);
                $this->modifications = $modifications;
                return;
            }
        }
    ...
    public function updateModificationsQty()
    {
        $this->setQuantity(array_sum(array_map(function (Modification $modification) {
            return $modification->quantity;
        }, $this->modifications)));
    ...
    private function updatePhotos(array $photos): void
    {
        ...
        $this->populateRelation('secondaryPhoto', next($photos));
    }
    ...
    public function revokeRelatedAssignment($id): void
    {
        $assignments = $this->relatedAssignments;
        foreach ($assignments as $i => $assignment) {
            if ($assignment->isForProduct($id)) {
    ...
    private function doWithReview($id, $callback): void
    {
        $reviews = $this->reviews;
    ...
    {
        return $this->hasMany(Photo::class, ['product_id' => 'id'])->orderBy('ord');
    }
    ...
    public function getSecondaryPhoto(): ActiveQuery
    {
        return $this->hasOne(Photo::class, ['id' => 'secondary_photo_id']);
    }

    public function getModifications(): ActiveQuery
    {
        return $this->hasMany(Modification::class, ['product_id' => 'id'])->orderBy('code');
    }
    ...
    {
        return $this->hasMany(Review::class, ['product_id' => 'id'])->having(['active' => 1]);
    }
    ...
    public function afterSave($insert, $changedAttributes): void
    {
        ...
        if (array_key_exists('secondaryPhoto', $related)) {
            $this->updateAttributes(['secondary_photo_id' => $related['secondaryPhoto'] ? $related['secondaryPhoto']->id : null]);
        }
    }
